    ...
    if model_type == 'BERT':
        ...
        for epoch in range(config['epochs']): 
            ...
            for batch, (X, y) in enumerate(train_dataloader): 
                ...
                pred_train = model.forward(X)  # pred_train.shape: [8, 2]
            ...
            with torch.no_grad():
                for X, y in test_dataloader: 
                    ...
                    pred_test = model.forward(X)
                    test_loss += loss_fn(pred_test, y).item()
                    correct += (pred_test.argmax(1) == y).type(torch.float).sum().item()

                    pred += pred_test.argmax(1).tolist()
